...
export function FounderProfileVisual({
  ...
  return (
    <div className="relative overflow-hidden rounded-[2.2rem] px-4 py-4 sm:px-5 sm:py-5">
      ...
      <div className="relative z-10 grid gap-4 lg:grid-cols-[0.9fr_1.1fr] lg:items-center xl:gap-6">
        <CardShell
          tone="hero"
          className="p-5 shadow-[0_30px_120px_rgba(2,12,27,0.56)] sm:p-6"
        >
        ...
        <div className="group/editorial relative isolate min-h-[23rem] overflow-hidden rounded-[2rem] border border-white/8 bg-slate-950/40 shadow-[0_30px_100px_rgba(2,12,27,0.32)] sm:min-h-[26rem] lg:min-h-[28rem]">
          <Image
            fill
            priority
            ...
            className="object-cover object-[center_16%] transition-transform duration-[1600ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-safe-image-drift"
          />
      ...
      <div className="relative z-10 mt-5 border-t border-white/8 pt-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {highlights.map((item) => (
            <article
              key={item.label}
              className="rounded-[1.35rem] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.018))] px-5 py-4 shadow-[0_18px_40px_rgba(2,12,27,0.12)] sm:px-6 sm:py-5"
            >
              <p className="text-[0.65rem] uppercase tracking-[0.26em] text-cyan-200/72">
                {item.label}
              </p>
