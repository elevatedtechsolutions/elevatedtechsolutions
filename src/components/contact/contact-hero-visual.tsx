...
export function ContactHeroVisual() {
  return (
    ...
    >
      ...
      <CardShell
        tone="hero"
        className="relative z-10 mx-auto max-w-[37rem] p-4 shadow-[0_30px_120px_rgba(2,12,27,0.58)] sm:p-5 lg:absolute lg:left-0 lg:top-14 lg:mx-0 lg:max-w-[32rem] xl:max-w-[35rem]"
      >
        ...
        <div className="grid gap-5 pt-5">
          <div className="rounded-[1.55rem] border border-white/8 bg-white/[0.025] p-5">
            <div className="grid gap-5 sm:grid-cols-[1.02fr_0.98fr] sm:items-start">
              ...
              <div className="space-y-3 border-t border-white/8 pt-4 sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0">
                {[
                  "Project type, budget, and timeline context",
                  "A clearer idea of fit before the next reply",
                  "Local roots with remote-friendly support"
                ].map((item) => (
                  <div
                    key={item}
                    className="flex items-start gap-3 text-sm leading-6 text-text-soft/82"
                  >
                    <span className="mt-[0.55rem] h-1.5 w-1.5 shrink-0 rounded-full bg-cyan-200/72" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
      ...
      <CardShell
        tone="quiet"
        className="motion-safe-float absolute bottom-6 right-4 z-20 hidden w-[17rem] p-4 lg:block"
      >
